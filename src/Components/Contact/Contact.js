...
class ContactUs extends React.Component{
  ...
  render(){ 
    ...
    const SocialNetworks = this.state.SocialNetworks.map(
      (index)=> {
        return (
    ...
    const Forms = this.state.Form.map(
      (index)=> { 
        ...
        return(
          <Col md={md}>
            <label  style={{color:"#fff"}} htmlFor={index.Name}> {index.Name}</label>
            <Input placeholder={index.Name} type={index.Type} rows={index.Rows}/>
          </Col>
        ); 
      }
    )
                    ...
                    <Container>
                      <Row>
                        <Col className="ml-auto mr-auto text-center" md="8">
                          <h2 className="title" style={{color:"#fff"}}>{this.state.Header}</h2>
                          <p style={{color:"#fff"}}>
